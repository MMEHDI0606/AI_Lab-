...
try:
    from inference import predict_answer, generate_distractors, get_hints, generate_question
    ...
    with col2:
        ...
        if st.button('🎲 Load random RACE sample'):
            ...
            if test_df is not None:
                ...
                # Only copy the article text — quiz is always generated by AI
        ...
        if not st.session_state.article.strip():
            ...
            with st.spinner('Generating quiz...'):
                ...
                try:
                    ...
                    # ── Always use full AI inference ──────────────────────────
                    # Step 1: AI generates the question + correct answer phrase
                    question, correct_answer = generate_question(article)

                    # Step 2: AI generates 3 distractors from the article
                    distractors = generate_distractors(article, question, correct_answer)

                    # Step 3: Build the 4 options and shuffle so correct answer
                    #         is not always in the same slot
                    opts = [correct_answer] + distractors[:3]
                    random.shuffle(opts)
                    correct_letter = 'ABCD'[opts.index(correct_answer)]

                    # Step 4: AI generates 3 graduated hints
